...
class TenantConfigRegistryTest {
    ...
    void testPolymorphicRouting_differentDataTypes() {
        // Arrange
        TenantConfig numericConfig = TenantConfig.withBucket(
                "test_ks", "numeric_table", List.of("pk"), "period_year", Set.of());
        TenantConfig stringConfig = TenantConfig.withBucket(
                "test_ks", "string_table", List.of("pk"), "period_year", Set.of());
    ...
    void testExists_returnsTrue_whenConfigExists() {
        // Arrange
        TenantConfig config = TenantConfig.withBucket(
                "test_ks", "test_table", List.of("pk"), "period_year", Set.of());
        registry.register("IBM", "DAILY", "NUMERIC", config);
    ...
    void testClear_removesAllConfigs() {
        // Arrange
        TenantConfig config1 = TenantConfig.withBucket(
                "test_ks", "table1", List.of("pk"), "period_year", Set.of());
        TenantConfig config2 = TenantConfig.withBucket(
                "test_ks", "table2", List.of("pk"), "period_year", Set.of());
